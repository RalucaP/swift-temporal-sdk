...
This article shows you how to run workflows against a local test server,
test signals, queries, and updates, skip time for long-running workflows,
and verify that code changes stay deterministic with replay testing. You can
test workflows in two ways:
...
The test trait starts the server before tests run and shuts it down after.
...
Given a workflow with signal, query, and update handlers:
...
setup is the same as the integration test above; inside the client closure:
...
### Replay test with WorkflowReplayer
...
> Tip: Common causes of replay failure include:
> - Reordering activity calls
> - Adding, removing, or changing timer durations
> - Branching on `Date()` or `UUID()` instead of workflow context APIs
> - Non-deterministic collection order (for example, iterating a dictionary)
...
> Tip: You can also capture history inline with
> ``WorkflowHandle/fetchHistory(waitNewEvent:eventFilterType:skipArchival:callOptions:)``
> after running a workflow in an integration
> test, and replay it immediately. No CLI export needed.
